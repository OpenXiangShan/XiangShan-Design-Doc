...
## Constraints
- Treat Chinese (`docs/zh/`) documents as source of truth. Do not modify Chinese content for translation/alignment purposes. The only allowed exception is fixing Chinese reference/label issues discovered by `tools/lint/ref-checker.py` during the required pre-check stage.
...
- Maintain strict line-count parity and line-by-line alignment between Chinese and English documents. The English version must keep the same number of lines as Chinese and preserve one-to-one structure for headings/paragraphs/lists. If existing English wording is already semantically equivalent to Chinese, keep it instead of re-translating, but still preserve line alignment. The only exception is inside tables, where extra or reduced line breaks are allowed for readability.
...
## Approach
1. **Determine scope**: If the user specified particular files, use those. Otherwise, inspect both uncommitted and recent committed Chinese changes (for example via `git status`, `git diff --name-only`, and recent `git log`/`git show`) to identify changed Chinese (`docs/zh/`) Markdown files. Focus on incremental changes rather than reprocessing unchanged content.
...
3. **Pre-check Chinese references first**: Before any English alignment work, check whether `tools/lint/ref-checker.py` exists. If it does, run it against the relevant Chinese file(s)/directory first, review stderr+stdout logs, fix genuine Chinese reference/label issues, and report the fixes to the user. If the script does not exist, perform a manual Chinese reference scan.
4. **Read both versions**: Read the Chinese file (source of truth) and the English file (target).
5. **Detect changes**: Identify sections, paragraphs, tables, lists, or cross-references that were added, removed, or modified in the Chinese version compared to the English version. Also detect any new or deleted `.index.md` or image files (e.g., under `figure/`) in the Chinese tree, as well as any new or deleted directories that change the overall document structure.
6. **Symlink management**: For any new `.index.md` or image files detected in the Chinese tree, create the corresponding symlinks:
...
7. **Directory structure change handling**: If new or deleted directories/files are detected in step 5, these are structural changes. Report them clearly.
...
8. **Align English doc**: Edit the English file to reflect the Chinese changes, while:
   ...
   - Prioritizing incremental updates from Chinese changes (uncommitted and recent commits); do not rewrite unchanged English passages just to make wording more literal.
...
9. **Cross-reference audit**: If the modified Chinese file contains links to other documents (e.g., `[text](other.md#anchor)` or `[@sec:other]`), check whether those linked documents also have uncommitted Chinese changes. If they do, align the linked English documents as well and report it.
10. **Post-edit validation**: After editing, verify that `sec/`, `fig/`, `tbl` labels in the English file are consistent and that no symlinked files were accidentally modified.
   ...
   - The script uses Python's `logging` module and writes its diagnostics to stderr, so inspect the terminal output as a whole rather than stdout only.
   - When the script accepts multiple paths, do not mix `docs/en/` and `docs/zh/` in the same invocation. Check each language tree separately to avoid expected duplicate-label reports between bilingual copies.
   ...
   - A `sec-label: No label at .../.index.md:1 (...)` warning is expected for directory index files. Safely ignore this warning.
...
11. **Report**: Summarize which files were aligned, what structural changes were made (new sections, removed paragraphs, updated tables, fixed cross-references), which Chinese pre-check/fix actions were performed, which symlinks were created or removed, whether any directory structure changes were detected and what updates may be needed for `.index.md` / `mkdocs-zh.yml` / `mkdocs-en.yml`, and note any linked documents that were also updated.
