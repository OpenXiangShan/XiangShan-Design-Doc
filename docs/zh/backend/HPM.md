...
| EVENT1         | 19:10  |       |                                               |          |
| EVENT0         | 9:0    |       | x: 对应的事件计数器对事件 x 计数              |          |
|                |        |       |                                               |          |
+----------------+--------+-------+-----------------------------------------------+----------+

(注：每周期组合or/and/xor 逻辑操作均为按位，add 为直接加法操作)

其中，计数器事件的组合方法为：

对于复合型性能事件，可能采用多个硬件事件组合而成：
...
对于独立事件：将组合操作配置为默认or，且仅配置一种事件，其余事件配置为 0 (空事件)。
...
| 名称       | 位域   | 读写  | 行为                                          | 复位值 |
+============+========+=======+===============================================+========+
| OFVEC      | 31:3   | RO    | mhpmeventx 寄存器上溢标志位:                  | 0      |
|            |        |       |                                               |        |
|            |        |       | 1： 发生上溢                                  |        |
|            |        |       |                                               |        |
|            |        |       | 0： 没有发生上溢                              |        |
...
scountovf 作为 mhpmevent 寄存器 OF 位的只读映射，受 xcounteren 控制:
