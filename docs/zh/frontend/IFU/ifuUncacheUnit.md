...
需要强调的是，`uncache` 取指是异步多拍完成的：IFU 在 S2 级向 `IfuUncacheUnit` 发出请求后，`s2_valid` 持续保持（并据此对 S1、S0 级施加反压），直到 `uncache` 通路返回数据，才在 S2 级完成数据拼接、RVC 扩展与入队；S3（写回）级仅在返回后向 FTQ 发起 uncache 重定向。因此缓存路径在 S1、S2 级基于指令数据计算的预译码结果均无法直接适用于 `uncache` 通路，需在其数据返回后重新计算。
...
`uncache` 总线每次均返回 64bit 对齐的数据。因此，当指令跨越 64bit 边界时，前端 `uncache` 通路会发送两次请求，IFU 无需感知。跨页时则不同：若跨页的是非 RVC（32 位）指令，则指令会拆分到两个页面，而两个页面的物理地址与权限属性可能并不一致，前端 `uncache` 通路无法自行处理第二个页面，会返回 `needResend` 标志，由 IFU 将本次取指流程走两遍完成拼接。
...
- 跨页：仅跨页的非 RVC（32 位）指令需要"走两遍"IFU。第一遍取指返回该指令的低 16 位，`needResend` 置位，IFU 暂存低 16 位数据并经 `uncacheRedirect` 重定向到下一个预测块；第二遍从下一预测块取回高 16 位后，按 half-RVI 拼接规则（见 [index.md](index.md)）恢复完整的 32 位指令。若预测器给出跳转结果，IFU 同样暂存半条指令数据，但会立即发送重定向，从正确的下一个预测块获得另一半数据。跨页的 RVC 指令完整落在第一页，单次取指即可完成。
- 非跨页：一条指令可在一个预测块中取完。
