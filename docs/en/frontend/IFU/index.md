...
| InstrUncache | Instruction Ucache Module                | Instruction MMIO Fetch Processing Unit                                |
...
| InstrUncache                | Instruction MMIO Fetch Processing Unit |
...
When the IFU receives a flush request from the BPU, it compares the pointers of
the fetch requests in the F0 and F1 pipeline stages with the pointer of the
flush request from the BPU. If the flushed pointer is before the fetch pointer,
it indicates that the current fetch request is on the wrong execution path and
requires a pipeline flush; otherwise, the IFU can ignore this flush request from
the BPU.

### Early branch prediction error check

To reduce flushes for some easily identifiable branch prediction errors, the IFU
uses pre-decode information generated in the F2 stage to perform front-end
branch prediction error checking in the F3 pipeline stage. The pre-decode
information is first sent to the PredChecker module, which checks for jal-type
errors, ret-type errors, invalid instruction prediction errors, and non-CFI
instruction prediction errors based on the CFI instruction type. Simultaneously,
it calculates 16 target addresses based on the instruction code and compares
them with the predicted target addresses to check for branch target address
errors. PredChecker corrects jal-type errors, erroneous ret predictions, and
erroneous jalr predictions, and regenerates the instruction valid range vector
fixedRange (where 1 indicates the instruction is within the predicted block).
Based on jump_range and ftr_range, fixedRange narrows the range from the start
address to the first undetected jal or ret instruction according to the check
results of jal and ret. The following are the types of branch prediction check
errors by the PredChecker module:

- jal-type error: A jal instruction exists within the range of the predicted
  block, but the predictor did not predict a jump for this instruction, or the
  predicted jump position of the block is after this instruction;
- jalr-type error: A jalr instruction exists within the range of the predicted
  block, but the predictor did not predict a jump for this instruction, or the
  predicted jump position of the block is after this instruction;
- ret-type error: A ret instruction exists within the range of the predicted
  block, but the predictor did not predict a jump for this instruction, or the
  predicted jump position of the block is after this instruction;
